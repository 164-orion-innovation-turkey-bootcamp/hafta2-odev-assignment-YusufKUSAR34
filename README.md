...
## 3 tane class oluşturdum.Bunun sebebi yapılan işlemi nesneye dayalı programlamaya döküp yapılacak işlemin anlaşılırlığını arttırmak.

# 1. Order class
 Order class'ı sipariş işlemlerini yönetiyor.Bu işlemleri gerçekleştiren metodlara sahip.Metodların gerçekleştirdiği işlemler açıklama satırların da detaylıca belirtildi.
# 2. Stock class
 Stock class'ı stok kontrolü yapan bir methoda sahip.
# 3. WorkProcess class
   WorkProcess class'ı iş sürecinin yönetilmesini sağlıyor.process adında bir methodu var bu method aldığı parametrelere göre asenkron bir programlama sağlayıp gönderilen sürelere göre iş sürecini sağlıyor.
## Proje de kullanıcının consoledan veri girişi yapması sağlanıyor.Kullanıcının console dan veri girişi yapabilmesi için (npm i --save console-read-write) paketi kuruldu.
